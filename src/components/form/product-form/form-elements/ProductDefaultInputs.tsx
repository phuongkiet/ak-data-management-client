...
const ProductDefaultInputs = ({ product, isCreateMode }: ProductProps) => {
  ...
  const handleWebsiteProductNameChange = () => {
    if (
      ...
      if (pattern?.name) {
        const newWebsiteProductName = `${productStore.productForm.autoBarCode} ${pattern.name} ${pattern.description}`;
  ...
  const handleOtherNoteChange = (value: string) => {
    setOtherNote(value);
    productStore.updateProductForm("otherNote", value);
  };
          ...
          <div className="grid grid-cols-2 gap-6">
            <div className="space-y-6">
              ...
              <div>
                ...
                <NumericFormat
                  value={
                    isCreateMode ? websiteProductPrice : product?.productPrice
                  }
                  thousandSeparator
                  prefix={appCurrency}
                  allowNegative={false}
                ...
                <div>
                  ...
                  <Input
                    ...
                    value={
                      isCreateMode
                        ? productStore.productForm.productOrderNumber
                        : product?.productOrderNumber || undefined
                    }
                  />
                </div>
                <div>
